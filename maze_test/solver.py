    ...
    def solve(self):
        ...
        while True:  # Loop until return statement is encountered
            ...
            while current_level:  # While still cells left to search on current level
                k_curr, l_curr = current_level.pop(
                    0
                )  # Search one cell on the current level
                self.maze.grid[k_curr][
                    l_curr
                ].visited = True  # Mark current cell as visited
                path.append(
                    ((k_curr, l_curr), False)
                ...
                if (
                    k_curr,
                    l_curr,
                ) == self.maze.exit_coor:  # Exit if current cell is exit cell
                    if not self.quiet_mode:
                        print("Number of moves performed: {}".format(len(path)))
                        print(
                            "Execution time for algorithm: {:.4f}".format(
                ...
                neighbour_coors = self.maze.validate_neighbours_solve(
                    ...
                    k_curr,
                    l_curr,
                    self.maze.exit_coor[0],
                    self.maze.exit_coor[1],
    ...
    def solve(self):
        ...
        while True:  # Loop until return statement is encountered
            neighbours_kl = self.maze.find_neighbours(
                k_curr, l_curr
            )  # Find neighbours for first search
            real_neighbours_kl = [
                neigh
                for neigh in neighbours_kl
                if not grid[k_curr][l_curr].is_walls_between(grid[neigh[0]][neigh[1]])
            ]
            neighbours_kl = [
                neigh
                for neigh in real_neighbours_kl
                if not grid[neigh[0]][neigh[1]].visited
            ]

            neighbours_pq = self.maze.find_neighbours(
                p_curr, q_curr
            )  # Find neighbours for second search
            real_neighbours_pq = [
                neigh
                for neigh in neighbours_pq
                if not grid[p_curr][q_curr].is_walls_between(grid[neigh[0]][neigh[1]])
            ]
            neighbours_pq = [
                neigh
                for neigh in real_neighbours_pq
                if not grid[neigh[0]][neigh[1]].visited
            ]
            ...
            if len(neighbours_kl) > 0:  # If there are unvisited neighbour cells
                ...
                (
                    k_curr,
                    l_curr,
                ) = backtrack_kl.pop()  # Pop previous visited cell (backtracking)
            ...
            if len(neighbours_pq) > 0:  # If there are unvisited neighbour cells
                ...
                (
                    p_curr,
                    q_curr,
                ) = backtrack_pq.pop()  # Pop previous visited cell (backtracking)
            ...
            if any((True for n_kl in real_neighbours_kl if (n_kl, False) in path_pq)):
                ...
                path = [
                    p_el for p_tuple in zip(path_kl, path_pq) for p_el in p_tuple
                ]  # Zip paths
                if not self.quiet_mode:
                    print("Number of moves performed: {}".format(len(path)))
                    print(
...
class DepthFirstBacktracker(Solver):
    ...
    def solve(self):
        ...
        while (
            k_curr,
            l_curr,
        ) != self.maze.exit_coor:  # While the exit cell has not been encountered
        ...
        if not self.quiet_mode:
            print("Number of moves performed: {}".format(len(path)))
            print(
                "Execution time for algorithm: {:.4f}".format(time.time() - time_start)
            )
